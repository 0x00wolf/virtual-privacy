...
def signed_message(bytes_string: bytes,
    ...
    Args:
        ...
        bytes_string (bytes): The encoded message.
    ...
    if verbose:
        print(f'[ ] Message successfully signed with {private_key}.')
    session_key = crypter.random_bytes(32)
    if verbose:
        print(f'[ ] Generating random 256-bit session key. First 10 bytes: '
              f'{session_key[:10]}')
    session_key, ciphertext = crypter.chacha20_poly1305.encrypt(
        bytes_string=bytes_string,
        session_key=session_key)
    if verbose:
        print("[ ] Message successfully encrypted with session key & "
              f"ChaCha20-Poly1305 stream-cipher. Ciphertext: {ciphertext}")
    wrapped_key = crypter.rsa.wrap(public_key=public_key,
                                   bytes_string=session_key)
    if verbose:
        print(f"[ ] Session key successfully wrapped with {receiver} "
              f"RSA public key. First 10 bytes: {wrapped_key[:10]}")
