...
def chat_server(database_path: str,
    ...
    try:
        ...
        try:
            buffer = conn.recv(396)
            signature = buffer[:384]
            vp_protocol_header = buffer[384:]
            if verbose:
                print(f"[*] Received 396 byte buffer, signature + VP header"
                      f" @ {datetime.now().strftime('%m/%d/%Y-%H:%M:%S')}"
            ...
            if verbose:
                print(f"[*] Received ciphertext of {len(ciphertext)} bytes.")
        except OSError as e:
            raise NetworkError(message=f"{e}")
