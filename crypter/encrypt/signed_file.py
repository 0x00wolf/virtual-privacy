...
def signed_file(file_in: str,
    ...
    Returns:
        wrapped_key (bytes | None): The 256-bit session key encrypted with the
        receiver's RSA public key.
        signature (bytes | None): The message signature created with the
        sender's RSA
        private key.
        ciphertext (str | None): The JSON result from the ChaCha20-Poly1305
        encryption
        process.
    ...
    try:
        ...
        print(f"[ ] File payload successfully written to: {file_out}")
    except OSError as e:
