...
PKI_OPERATIONS = ['self-sign', 'ss', 'root-ca', 'ca', 'fast-gen', 'fg']
PKI_ASSIGNMENTS = {
    'ss': 'self-sign',
    'ca': 'root-ca',
    'fg': 'fast-gen',
...
def parse():
    ...
    mode_group.add_argument(
        ...
        '\n* Optional: --private-key & --public-key.'
        '\n'
    )
    mode_group.add_argument(
        '--generate-pki', '-pki',
    ...
    mode_group.add_argument(
        ...
        help='[*] VPP file encryption operations'
             '\n[-] Operations:'
             '\nfile | f    # Encrypt a file (file-in) to a specified '
    ...
    # Verify that the provided file-in is a legitimate file
    avoid = ['dir', 'path']
    if args.encrypt in avoid or args.decrypt in avoid:
        pass
    else:
        if args.file_in:
    ...
    # Handle 'file' VPP encryption path requirements.
    if args.encrypt == 'file':
        if not args.file_in:
            raise VPRuntimeError(message="Missing: --file-in, -fi"
                                         "\n[ ] The path to a file is required"
                                         " for VPP encryption.")
        if not args.file_out:
            raise VPRuntimeError(message="Missing: --file-out, -fo"
                                         "\n[ ] The path to export the "
                                         "encrypted file is required for VPP "
                                         "encryption.")

    # Handle 'file' VPP decryption path requirements.
    if args.decrypt == 'file':
        if not args.file_in:
            raise VPRuntimeError(message="Missing: --file-in."
                                         "\n[ ] The path to the encrypted file"
                                         " is required for VPP decryption.")
        if not args.file_out:
            raise VPRuntimeError(message="Missing: --file-out."
                                         "\n[ ] The path to export the "
                                         "decrypted file is required for VPP "
                                         "decryption.")

    # Handle 'dir' VPP encryption path requirements.
    if args.encrypt == 'dir':
        if not args.file_in:
            raise VPRuntimeError(message="Missing: --file-in | -fi"
                                         "\n[ ] The path to the directory "
                                         "selected for VPP encryption is "
                                         "required.")

    # Handle 'dir' VPP decryption path requirements
    if args.decrypt == 'dir':
        if not args.file_in:
            raise VPRuntimeError(message="Missing: --file-in | -fi"
                                         "\n[ ] The path to the encrypted "
                                         "directory is required for VPP "
                                         "decryption.")

    if args.encrypt == 'path':
        if not args.file_in:
            raise VPRuntimeError(message="Missing: --file-in | -fi"
                                         "\n[ ] VPP path encryption requires "
                                         "a path to encrypt...")

    if args.decrypt == 'path':
        if not args.file_in:
            raise VPRuntimeError(message="Missing: --file-in | -fi"
                                         "\n[ ] VPP path decryption requires "
                                         "a path to decrypt...")

    # Handle VPP encryption credential requirements.
    if args.encrypt:
        if not args.private_key:
            raise VPRuntimeError(message="Missing: --private-key | -pr"
                                         "\n[ ] The path to an RSA private key"
                                         " is required for VPP "
                                         "encryption & decryption.")
        if not args.public_key:
            raise VPRuntimeError(message="Missing: --public-key | -pr"
                                         "\n[ ] The path to an RSA public key "
                                         "is required for VPP encryption & "
                                         "decryption")

    # Handle VPP decryption credential requirements.
    if args.decrypt:
        if not args.private_key:
            raise VPRuntimeError(message="Missing: --private-key, -pr"
                                         "\n[ ] The path to an RSA private "
                                         "key is required for VPP encryption.")
        if not args.public_key:
            raise VPRuntimeError(message="Missing: --public-key, -pu"
                                         "\n[ ] The path to an RSA public key "
                                         "is required for VPP encryption.")
    ...
    if args.server == 'ftp':
        if not args.file_in:
            raise VPRuntimeError(message="Server operation FTP requires the "
                                         "path to a file in.")
    if args.client == 'ftp':
        if not args.file_out:
            raise VPRuntimeError(message="Client FTP operation requires a "
                                         "file out path.")

    # Handle Generate PKI argument requirements
    if (args.generate_pki == 'self-sign' and not args.private_key or
            args.generate_pki == 'self-sign' and not args.certificate):
        raise VPRuntimeError(
            message=f"Missing required arguments for X509 self-signing.\n"
                    f"[ ] Operation requires: --private-key & --certificate")
    if args.generate_pki == 'fast-gen' and not args.private_key:
        args.private_key = 'key.pem'
    if args.generate_pki == 'fast-gen' and not args.certificate:
        args.certificate = 'cert.pem'

    # Handle Database argument requirements
    if args.database == 'add-key':
        if not args.public_key:
            raise VPRuntimeError(
                message=
                "Operation requires an RSA public key to store in the "
                "database."
            )
    if args.database == 'add-server':
        if not args.host or not args.port or not args.target:
            raise VPRuntimeError(
                message="Operation requires host, port, and target nickname.\n"
                        "[ ] Certificate and RSA public key are optional."
            )
    if args.database == 'show-server':
        if not args.target:
            raise VPRuntimeError(
                message="Operation requires a target ID or nickname."
            )
    if args.database == 'show-key':
        if not args.target:
            raise VPRuntimeError(
                message="Operation requires a target ID or nickname."
            )
    if args.database == 'delete-key':
        if not args.target:
            raise VPRuntimeError(
                message="Operation requires a target nickname or ID."
            )
    if args.database == 'delete-server':
        if not args.target:
            raise VPRuntimeError(
                message="Operation requires a target nickname or ID."
            )
